...
def plot_convergence_steps_vs_discount_factor(title, df, xlabel="Discount factor", ylabel="Steps to converge"):
    plt.close()
    plt.figure()
    plt.title(title)
    plt.xlabel(xlabel)
    ...
    plt.plot(df['discount_factor'], df['convergence_steps'], '-', linewidth=1)
    plt.legend(loc="best")

    return watermark(plt)

def plot_time_to_converge_vs_discount_factor(title, df, xlabel="Discount factor", ylabel="Time to converge"):
    plt.close()
    plt.figure()
    plt.title(title)
    plt.xlabel(xlabel)
    ...
    plt.plot(df['discount_factor'], df['elapsed_time'], '-', linewidth=1)
    plt.legend(loc="best")

    return watermark(plt)

def plot_reward_means_vs_discount_factor(title, df, xlabel="Discount factor", ylabel="Mean rewards"):
    plt.close()
    plt.figure()
    plt.title(title)
    plt.xlabel(xlabel)
    ...
    plt.plot(df['discount_factor'], df['reward_mean'], '-', linewidth=1)
    plt.legend(loc="best")
...
def plot_reward_and_delta_vs_steps(title, df, xlabel="Steps", ylabel="Reward"):
    ...
    lns1 = ax.plot(df.index.values, df['reward'], linewidth=1, label=ylabel, color='blue')

    ex_ax = ax.twinx()
    lns2 = ex_ax.plot(df.index.values, df['delta'], linewidth=1, label='Delta', color='brown')
...
def find_optimal_params(problem_name, base_dir, file_regex):
    ...
    for f in grid_files:
        ...
        if problem_name == 'VI' or problem_name == 'PI':
            df = df.join(df['params'].apply(json.loads).apply(pd.Series))

            p = plot_convergence_steps_vs_discount_factor(
                title='{}: Steps to converge VS Discount factor'.format(readable_mdp), df=df)
            file_name = '{}/{}/{}_convergence_steps_discount_factor.png'.format(REPORT_PATH, problem_name, mdp)
            p.savefig(file_name, format='png', dpi=150)
            p.close()

            p = plot_time_to_converge_vs_discount_factor(title='{}: Time to converge VS Discount factor'.format(readable_mdp), df=df)
            file_name = '{}/{}/{}_time_to_converge_discount_factor.png'.format(REPORT_PATH, problem_name, mdp)
            p.savefig(file_name, format='png', dpi=150)
            p.close()

            p = plot_reward_means_vs_discount_factor(
                title='{}: Mean rewards VS Discount factor'.format(readable_mdp), df=df)
            file_name = '{}/{}/{}_mean_rewards_discount_factor.png'.format(REPORT_PATH, problem_name, mdp)
            p.savefig(file_name, format='png', dpi=150)
            p.close()
